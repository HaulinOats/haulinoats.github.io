...
const initUtility = () => {
  ...
  Object.keys(cropData).forEach((seedIdx) => {
    ...
    _CropGrowthSeasons.forEach((season) => {
      totalSeasonCrops[season].push(
        harvestData[_CropHarvestIndex]?.split("/")[0] +
          " - " +
          _CropHarvestIndex
      );
    });
