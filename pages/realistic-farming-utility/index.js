...
const initUtility = () => {
  ...
  //regrowth capable crops will sell for less
  let regularSellPriceMultiplier = generateRandomWhole(12, 18);
  let regrowthSellPriceMultiplier = (6, 10);
  ...
  for (const season in seasonCrops) {
    ...
    seasonCropPool.forEach((seedIdx) => {
      ...
      let cropSeasons = item.cropData[1]
        ...
        .join(", ");
      let seasonText = `Plant these in ${cropSeasons.replaceLast(",", " or")}. `;
      ...
      itemData[seedIdx] = {
        cropData: item.cropData.join("/"),
        seedData: item.seedData.join("/"),
        objectData: item.objectData.join("/"),
      };
    });
    console.log("----------------------");
  }

  console.log("Compiled Item Data:");
