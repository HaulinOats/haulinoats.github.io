window.addEventListener("load", function () {
  ...
  const optionsContainer: HTMLElement = document.querySelector(
  ...
  const optionInputs: NodeList = document.querySelectorAll(
    ".grid-options-container input"
  );
  ...
  for (let param in optionsParams) {
    ...
    switch (paramName) {
      case "opacityDuration":
        ...
        (<HTMLInputElement>(
          document.querySelector("#opacity-duration-input")
        )).value = options["opacityDuration"];
        break;
      case "pageChangeTimeout":
        options["pageChangeTimeout"] = Number(paramValue);
        (<HTMLInputElement>(
          document.querySelector("#page-change-timeout-input")
        )).value = options["pageChangeTimeout"];
        break;
      case "kenBurnsScaling":
        options["allowKenBurnsScaling"] = paramValue === "0" ? false : true;
        (<HTMLInputElement>(
          document.querySelector("#ken-burns-scaling-input")
        )).checked = options["allowKenBurnsScaling"];
        break;
      case "gridLayoutsId":
        options["gridLayoutsId"] = Number(paramValue);
        (<HTMLInputElement>(
          document.querySelector("#grid-layouts-id-input")
        )).value = options["gridLayoutsId"];
        break;
    }
  }
  ...
  function gridScreensaver(attachElement, imageArr, options) {
    ...
    let opacityDuration =
      "opacityDuration" in options ? Number(options.opacityDuration) : 1;
    let pageChangeTimeout =
      "pageChangeTimeout" in options ? Number(options.pageChangeTimeout) : 40;
    let allowKenBurnsScaling =
      "allowKenBurnsScaling" in options
        ? Boolean(options.allowKenBurnsScaling)
        : true;
    let closeOnClick =
      "closeOnClick" in options ? Boolean(options.closeOnClick) : false;
    let bindStartQuerySelector =
      "bindStartQuerySelector" in options && options.bindStartQuerySelector
        ? options.bindStartQuerySelector
        : null;
    let startCallback =
      "startCallback" in options && typeof options.startCallback === "function"
        ? options.startCallback
        : false;
    let loadCallback =
      "loadCallback" in options && typeof options.loadCallback === "function"
        ? options.loadCallback
        : false;
    let stopCallback =
      "stopCallback" in options && typeof options.stopCallback === "function"
        ? options.stopCallback
        : false;
    let gridLayoutsId =
      "gridLayoutsId" in options ? Number(options.gridLayoutsId) : 0;
    ...
    function buildScreensaverAndApplyListeners() {
      ...
      screensaverFront = document.getElementById(
        "mvwsct_ui_gridScreensaverInner_frontGrid"
      );
      screensaverBack = document.getElementById(
      ...
      screensaverFront.addEventListener(
        "transitionend",
        handleTransition,
        false
      );
      ...
      if (bindStartQuerySelector) {
        document
          .querySelector(bindStartQuerySelector)
    ...
    function handleTransition(e) {
      ...
      if (
        e.target.classList.contains(
          "mvwsct_ui_gridScreensaverInner_gridItem_inner"
        )
      ) {
        switch (e.propertyName) {
          ...
          case "letter-spacing":
            ...
            if (
              ...
              e.target.previousElementSibling.style.backgroundImage = `url(${handleCountAndReturnImageUrl(
                parentEl.getAttribute("data-orientation")
              )})`;
              e.target.style.opacity = "0";
            } else {
              //if back image, fade-in front image
              e.target.nextElementSibling.style.backgroundImage = `url(${handleCountAndReturnImageUrl(
                parentEl.getAttribute("data-orientation")
              )})`;
    ...
    function preloadAndSortImages() {
      ...
      for (let i = 0; i < imageArr.length; i++) {
        ...
        tempImg.onload = () => {
          ...
          imagesLeftToLoad--;
          if (!imagesLeftToLoad) {
            gridImageData.horizontal.imageArr = shuffleArr(
              gridImageData.horizontal.imageArr
            );
            gridImageData.vertical.imageArr = shuffleArr(
              gridImageData.vertical.imageArr
            );
            gridImageData.square.imageArr = shuffleArr(
              gridImageData.square.imageArr
            );
    ...
    function buildGrids(screensaverContainer) {
      ...
      for (let i = 0; i < gridData.length; i++) {
        let isFront = parseInt(
          screensaverContainer.getAttribute("data-is-front")
        );
      ...
      setTimeout(() => {
        ...
        let gridItems = screensaverContainer.querySelectorAll(
          ".mvwsct_ui_gridScreensaverInner_gridItem"
        );
        for (let i = 0; i < gridItems.length; i++) {
          let gridItemRatio =
            gridItems[i].offsetWidth > gridItems[i].offsetHeight
              ? gridItems[i].offsetHeight / gridItems[i].offsetWidth
              : gridItems[i].offsetWidth / gridItems[i].offsetHeight;
    ...
    function initGridAnimations(screensaverContainer) {
      ...
      gridElements = shuffleArr(
        [].slice.call(
          screensaverContainer.querySelectorAll(
            ".mvwsct_ui_gridScreensaverInner_gridItem"
          )
        )
      );
      ...
      for (let i = 0; i < gridElements.length; i++) {
        ...
        let frontImage: HTMLElement = gridElements[i].querySelector(
          ".mvwsct_ui_gridScreensaverInner_gridItem_front"
        )!;
        let orientation = gridElements[i].getAttribute("data-orientation");
        frontImage.style.backgroundImage = `url(${handleCountAndReturnImageUrl(
          orientation
        )})`;
        switch (i) {
    ...
    function applyTransitionAttributes(el, shouldScale, firstLoad) {
      ...
      if (shouldScale) {
        ...
        setTimeout(() => {
          el.style.transition = `letter-spacing ${
            pageChangeTimeout / gridsPerPage
          }s ease-in-out ${transitionDelay}s`;
    ...
    function handleCountAndReturnImageUrl(imgOrientation) {
      ...
      if (
        gridImageData[orientation].imageArr[gridImageData[orientation].countIdx]
      ) {
        imageUrl =
          gridImageData[orientation].imageArr[
    ...
    function stop() {
      ...
      let allGridItems: NodeList = document.querySelectorAll(
        ".mvwsct_ui_gridScreensaverInner_gridItem_inner"
      );
    ...
    function unloadScreensaver() {
      ...
      let pageContainers = document.querySelectorAll(
        ".mvwsct_ui_gridScreensaverInnerContainer"
      );
