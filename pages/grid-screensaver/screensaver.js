window.addEventListener("load", function () {
    ...
    //build images array
    for (var i = 1; i < 97; i++) {
        imagesArr.push("./img/".concat(i, ".jpg"));
    }
    ...
    function gridScreensaver(attachElement, imageArr, options) {
        ...
        var allowKenBurnsScaling = "allowKenBurnsScaling" in options
            ? Boolean(options.allowKenBurnsScaling)
            : true;
        var closeOnClick = "closeOnClick" in options ? Boolean(options.closeOnClick) : false;
        var bindStartQuerySelector = "bindStartQuerySelector" in options && options.bindStartQuerySelector
            ? options.bindStartQuerySelector
            : null;
        var startCallback = "startCallback" in options && typeof options.startCallback === "function"
            ? options.startCallback
            : false;
        var loadCallback = "loadCallback" in options && typeof options.loadCallback === "function"
            ? options.loadCallback
            : false;
        var stopCallback = "stopCallback" in options && typeof options.stopCallback === "function"
                ...
                switch (e.propertyName) {
                    case "opacity":
                        ...
                        if (gridElements.length) {
                            ...
                            applyTransitionAttributes(nextEl === null || nextEl === void 0 ? void 0 : nextEl.lastElementChild, false, false);
                        }
                        else {
                    ...
                    case "letter-spacing":
                        ...
                        e.target.style.transition = "opacity ".concat(opacityDuration, "s");
                        //if front image, fade-out front image
                        if (e.target.classList.contains("mvwsct_ui_gridScreensaverInner_gridItem_front")) {
                            e.target.previousElementSibling.style.backgroundImage = "url(".concat(handleCountAndReturnImageUrl(parentEl.getAttribute("data-orientation")), ")");
                            e.target.style.opacity = "0";
                        }
                        else {
                            //if back image, fade-in front image
                            e.target.nextElementSibling.style.backgroundImage = "url(".concat(handleCountAndReturnImageUrl(parentEl.getAttribute("data-orientation")), ")");
        ...
        function preloadAndSortImages() {
            ...
            var _loop_2 = function (i) {
                ...
                tempImg.onload = function () {
                    var imageRatio = tempImg.width > tempImg.height
        ...
        function buildGrids(screensaverContainer) {
            ...
            for (var i = 0; i < gridData.length; i++) {
                ...
                html += "<div data-is-front='".concat(isFront, "' data-left='").concat(gridData[i].left, "' data-top='").concat(gridData[i].top, "' data-col='").concat(gridData[i].col, "' data-row='").concat(gridData[i].row, "' class='mvwsct_ui_gridScreensaverInner_gridItem'>\n                <div class='mvwsct_ui_gridScreensaverInner_gridItem_inner mvwsct_ui_gridScreensaverInner_gridItem_back'></div>\n                <div class='mvwsct_ui_gridScreensaverInner_gridItem_inner mvwsct_ui_gridScreensaverInner_gridItem_front'></div>\n              </div>");
            }
            ...
            setTimeout(function () {
                ...
                for (var i = 0; i < gridItems.length; i++) {
                    var gridItemRatio = gridItems[i].offsetWidth > gridItems[i].offsetHeight
        ...
        function initGridAnimations(screensaverContainer) {
            ...
            for (var i = 0; i < gridElements.length; i++) {
                ...
                frontImage.style.backgroundImage = "url(".concat(handleCountAndReturnImageUrl(orientation_1), ")");
                switch (i) {
        ...
        function applyTransitionAttributes(el, shouldScale, firstLoad) {
            ...
            if (shouldScale) {
                ...
                setTimeout(function () {
                    el.style.transition = "transform ".concat(pageChangeTimeout, "s ease-in-out ").concat(transitionDelay, "s");
                    el.style.transform = "scale3d(1,1,1)";
                }, 100);
            }
            else {
                setTimeout(function () {
                    el.style.transition = "letter-spacing ".concat(pageChangeTimeout / gridsPerPage, "s ease-in-out ").concat(transitionDelay, "s");
        ...
        function handleCountAndReturnImageUrl(imgOrientation) {
            ...
            if (gridImageData[orientation].imageArr[gridImageData[orientation].countIdx]) {
                imageUrl =
                    gridImageData[orientation].imageArr[gridImageData[orientation].countIdx].src;
        ...
        function unloadScreensaver() {
            ...
            for (var i = 0; i < pageContainers.length; i++) {
                while (pageContainers[i].lastChild) {
                    (_a = pageContainers[i].lastChild) === null || _a === void 0 ? void 0 : _a.remove();
                }
